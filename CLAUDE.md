...
99988. **Generator stop semantics.** The Generator stops only when JUDGE returns PASS OR when MAX_ITERATIONS is exhausted; the latter yields best-iteration output with `verifier_last_verdict=MAX_REACHED_RETURN`, not infinite escalation and not a hard blocker. The Generator must not self-approve; only the JUDGE PASS or the configured cap can close a run.
