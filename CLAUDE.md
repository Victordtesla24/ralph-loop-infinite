...
99989. **Generator never stops — ever.** The implementation agent is the Generator. It must generate, receive critique, generate again — until the verifier (NOT the generator) returns PASS. The Generator cannot declare it is done, cannot say "task complete", cannot say "finished", cannot say "all done", cannot self-assess that it has produced enough. Every FAIL from the verifier is a signal to the Generator to continue. The Generator ONLY stops when the verifier says STOP.
99988. **Generator = infinite loop by design.** The word "Generator" in this context means: produce output → receive verdict → if verdict = FAIL → produce improved output → repeat indefinitely. There is no iteration cap on the Generator. The only caps are MAX_ITERATIONS (hard blocker for the loop owner, not the Generator) and convergence (escalation, not exit). The Generator has no exit mechanism. The Generator must not implement, suggest, or attempt an exit mechanism.
